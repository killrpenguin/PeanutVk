...
namespace Peanut {
    ...
    namespace Utils {
        constexpr static std::uint32_t GENERIC_WIN_ERROR{ 0xffffffff };

        static auto GetVulkanMemoryType( VkMemoryPropertyFlags properties, std::uint32_t type_bits ) -> std::uint32_t {
            ...
            return GENERIC_WIN_ERROR;
        }

        static constexpr auto BytesPerPixel( ImageFormat format ) noexcept -> std::uint32_t {
            switch ( format ) {
        ...
        static constexpr auto WalnutFormatToVulkanFormat( ImageFormat format ) noexcept -> VkFormat {
            switch ( format ) {
    ...
    Image::Image( std::string_view path ) noexcept
        : m_Filepath( path ) {
        int width{};
        int height{};
        int channels{};
    ...
    Image::Image( std::uint32_t width, std::uint32_t height, ImageFormat format, const void* data ) noexcept
    ...
    auto Image::AllocateMemory( [[maybe_unused]] uint64_t size ) noexcept -> void {
        ...
        m_DescriptorSet = static_cast< VkDescriptorSet >(
    ...
    auto Image::SetData( const void* data ) noexcept -> void {
        VkDevice device = Application::GetDevice();
        ...
        // Copy to Image
        {
            ...
            vkCmdPipelineBarrier(
                command_buffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1,
                &copy_barrier );
            ...
            vkCmdPipelineBarrier(
                command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0,
                NULL, 1, &use_barrier );
    ...
    auto Image::Resize( std::uint32_t width, std::uint32_t height ) noexcept -> void {
