...
//
// Adapted from Dear ImGui Vulkan example and StudioCherno's Walnut Framework.
//
...
// This static_cast version of the IM_ARRAYSIZE keeps my compiler from complaining.
// That's the only reason we don't use ImGui's IM_ARRAYSIZE() macro.
#define MODERN_IM_ARRAYSIZE( _ARR ) ( static_cast< int >(( sizeof( _ARR ) / sizeof( *( _ARR ) ) )) )
...
static VKAPI_ATTR VkBool32 VKAPI_CALL debug_report(
    [[maybe_unused]] VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType,
    [[maybe_unused]] uint64_t object, [[maybe_unused]] size_t location, [[maybe_unused]] int32_t messageCode,
    ...
    fmt::println(
        ...
        string_VkDebugReportObjectTypeEXT( objectType ), pMessage );

    return VK_FALSE;
}
#endif // IMGUI_VULKAN_DEBUG_REPORT

static auto SetupVulkan( const char** extensions, uint32_t extensions_count ) -> void {
    VkResult err;
    ...
    // Create Descriptor Pool
    {
        ...
        pool_info.maxSets                    = 1000 * MODERN_IM_ARRAYSIZE( pool_sizes );
        pool_info.poolSizeCount              = static_cast< uint32_t >( MODERN_IM_ARRAYSIZE( pool_sizes ) );
...
static void SetupVulkanWindow( ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height ) {
    ...
    wd->SurfaceFormat                              = ImGui_ImplVulkanH_SelectSurfaceFormat(
        ...
        static_cast< std::size_t >( MODERN_IM_ARRAYSIZE( requestSurfaceImageFormat ) ), requestSurfaceColorSpace );
    ...
    wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(
        g_PhysicalDevice, wd->Surface, &present_modes[0], MODERN_IM_ARRAYSIZE( present_modes ) );
...
static auto FrameRender( ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data ) -> void {
    VkResult err;
    ...
    }
    {
        ...
        if ( allocatedCommandBuffers.size() > 0 ) {
            vkFreeCommandBuffers(
                g_Device, fd->CommandPool, static_cast< uint32_t >( allocatedCommandBuffers.size() ),
...
static auto FramePresent( ImGui_ImplVulkanH_Window* wd ) -> void {
...
namespace Peanut {
    Application::Application( const ApplicationSpecification& specification ) noexcept
    ...
    constexpr auto Application::Get() noexcept -> Application& {
        return *s_Instance;
    }
    auto Application::Init() noexcept -> void {
        // Setup GLFW window
        glfwSetErrorCallback( glfw_error_callback );
        if ( !glfwInit() ) {
            fmt::println( stderr, "Could not initalize GLFW!" );
            return;
        }
        ...
        if ( !glfwVulkanSupported() ) {
            fmt::println( stderr, "GLFW: Vulkan not supported!" );
            return;
        }
        ...
        SetupVulkanWindow( wd, surface, fb_width, fb_height );
        ...
        while ( !glfwWindowShouldClose( m_WindowHandle ) && m_Running ) {
            ...
            if ( g_SwapChainRebuild ) {
                ...
                if ( width > 0 && height > 0 ) {
                    ...
                    ImGui_ImplVulkanH_CreateOrResizeWindow(
                        g_Instance, g_PhysicalDevice, g_Device, &g_MainWindowData, g_QueueFamily, g_Allocator, width,
...

            {
                ...
                if ( p_io.ConfigFlags & ImGuiConfigFlags_DockingEnable ) {
    ...
    auto Application::GetTime() const noexcept -> float {
        return static_cast< float >( glfwGetTime() );
    }

    auto Application::GetInstance() noexcept -> VkInstance {
        return g_Instance;
    }

    auto Application::GetPhysicalDevice() noexcept -> VkPhysicalDevice {
        return g_PhysicalDevice;
    }

    auto Application::GetDevice() noexcept -> VkDevice {
        return g_Device;
    }

    auto Application::GetCommandBuffer( [[maybe_unused]] bool begin ) noexcept -> VkCommandBuffer {
        ImGui_ImplVulkanH_Window* wd = &g_MainWindowData;
    ...
    auto Application::FlushCommandBuffer( VkCommandBuffer commandBuffer ) noexcept -> void {
    ...
    auto Application::SubmitResourceFree( std::function< void() >&& func ) noexcept -> void {
